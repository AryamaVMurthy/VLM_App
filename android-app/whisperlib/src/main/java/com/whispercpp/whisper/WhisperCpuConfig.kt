...
object WhisperCpuConfig {
    val preferredThreadCount: Int
        // Empirically, 4-6 threads gives lower latency for tiny.en on ARM big.LITTLE SoCs.
        get() = CpuInfo.getRecommendedThreadCount()
}
...
private class CpuInfo(private val lines: List<String>) {
    ...
    companion object {
        ...
        fun getRecommendedThreadCount(): Int {
            val highPerf = getHighPerfCpuCount().coerceAtLeast(2)
            val available = Runtime.getRuntime().availableProcessors().coerceAtLeast(2)
            val bounded = minOf(8, available).coerceAtLeast(4)
            Log.i(
                LOG_TAG,
                "recommended whisper thread count=$bounded high_perf=$highPerf available=$available",
            )
            return bounded
        }

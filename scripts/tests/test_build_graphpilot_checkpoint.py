...
class BuildGraphPilotCheckpointTest(unittest.TestCase):
    ...
    def test_main_allows_checkpoint_without_artifact_pack_summary(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = pathlib.Path(tmp)
            paths = {}
            for name in (
            ...
            truth_pdf.write_bytes(b"%PDF-1.7\n%test\n")
            open_beads = root / "open_beads.json"
            open_beads.write_text("[]", encoding="utf-8")
            closed_beads = root / "closed_beads.json"
            closed_beads.write_text("[]", encoding="utf-8")
            output_root = root / "out"
            ...
            self.assertEqual(len(summaries), 1)
            payload = json.loads(summaries[0].read_text(encoding="utf-8"))
            self.assertIsNone(payload["canonical_evidence_paths"]["artifact_pack_summary"])

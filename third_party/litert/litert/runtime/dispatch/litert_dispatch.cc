...
#if !defined(LITERT_WINDOWS_OS)
LiteRtStatus LoadSiblingLiteRtRuntimeAsGlobal(
    const std::string& dispatch_library_path) {
  const auto slash = dispatch_library_path.find_last_of('/');
  if (slash == std::string::npos) {
    LITERT_LOG(LITERT_ERROR,
               "Invalid dispatch library path (cannot locate parent dir): %s",
               dispatch_library_path.c_str());
    return kLiteRtStatusErrorRuntimeFailure;
  }
  const std::string runtime_path =
      dispatch_library_path.substr(0, slash + 1) + "libLiteRt.so";
  void* handle = dlopen(runtime_path.c_str(), RTLD_NOW | RTLD_GLOBAL);
  if (!handle) {
    LITERT_LOG(
        LITERT_ERROR,
        "Failed to load LiteRT runtime '%s' with RTLD_GLOBAL: %s. "
        "Remediation: package ABI-compatible libLiteRt.so beside dispatch.",
        runtime_path.c_str(), dlerror());
    return kLiteRtStatusErrorRuntimeFailure;
  }
  LITERT_LOG(LITERT_INFO, "Loaded LiteRT runtime with RTLD_GLOBAL: %s",
             runtime_path.c_str());
  return kLiteRtStatusOk;
}

LiteRtStatus PromoteCallerLibraryToGlobal() {
  Dl_info info{};
  if (!dladdr(reinterpret_cast<void*>(&LiteRtDispatchInitialize), &info) ||
      !info.dli_fname) {
    LITERT_LOG(LITERT_WARNING,
               "Failed to resolve caller shared library for global promotion");
    return kLiteRtStatusErrorRuntimeFailure;
  }
  void* handle =
  ...
  if (!handle) {
    LITERT_LOG(LITERT_WARNING,
               "RTLD_NOLOAD global promotion failed for '%s': %s. Retrying "
               "without NOLOAD.",
               info.dli_fname, dlerror());
    handle = dlopen(info.dli_fname, RTLD_NOW | RTLD_GLOBAL);
  }
  if (!handle) {
    LITERT_LOG(LITERT_ERROR, "Failed to globally promote '%s': %s",
               info.dli_fname, dlerror());
    return kLiteRtStatusErrorRuntimeFailure;
  }
  LITERT_LOG(LITERT_INFO, "Promoted caller library to RTLD_GLOBAL: %s",
             info.dli_fname);
  return kLiteRtStatusOk;
}
#endif  // !defined(LITERT_WINDOWS_OS)
...
#if !defined(LITERT_WINDOWS_OS)
  LITERT_RETURN_IF_ERROR(LoadSiblingLiteRtRuntimeAsGlobal(shared_lib_path));

  // Qualcomm dispatch shared library references LiteRt* runtime symbols.
  // Promote the host LiteRT runtime library to the global group so bionic can
  // resolve those symbols when loading dispatch.
  LITERT_RETURN_IF_ERROR(PromoteCallerLibraryToGlobal());
#endif  // !defined(LITERT_WINDOWS_OS)

  auto dispatch_shared_library_or = litert::SharedLibrary::Load(
      shared_lib_path, litert::RtldFlags::Now().Local());
  if (!dispatch_shared_library_or) {
    LITERT_LOG(LITERT_ERROR,
               "Failed to load dispatch shared library '%s': %s",
               shared_lib_path.c_str(),
               dispatch_shared_library_or.Error().Message().c_str());
    return dispatch_shared_library_or.Error().Status();
  }
  *DispatchSharedLibrary = std::move(dispatch_shared_library_or.Value());
  LITERT_LOG(LITERT_INFO, "Dispatch shared library loaded successfully.");

  using LiteRtDispatchGetApi_t = LiteRtStatus (*)(LiteRtDispatchApi*);
  auto dispatch_get_api_or =
      DispatchSharedLibrary->LookupSymbol<LiteRtDispatchGetApi_t>(
          "LiteRtDispatchGetApi");
  if (!dispatch_get_api_or) {
    LITERT_LOG(LITERT_ERROR,
               "Failed to resolve LiteRtDispatchGetApi symbol: %s",
               dispatch_get_api_or.Error().Message().c_str());
    return dispatch_get_api_or.Error().Status();
  }
  auto LiteRtDispatchGetApi = *dispatch_get_api_or;

  if (auto status = LiteRtDispatchGetApi(&TheApi); status != kLiteRtStatusOk) {
    LITERT_LOG(LITERT_ERROR, "LiteRtDispatchGetApi returned status=%d",
               static_cast<int>(status));
    return status;
  }
  LITERT_LOG(LITERT_INFO, "LiteRtDispatchGetApi succeeded.");
  ...
  if (status != kLiteRtStatusOk) {
    LITERT_LOG(LITERT_ERROR, "Dispatch initialize(env, options) failed: %d",
               static_cast<int>(status));
  }
  if (status == kLiteRtStatusOk) {

...
LiteRtStatus QnnManager::Init(std::optional<std::string> shared_library_dir,
  ...
  auto ResolveLibraryPath = [&](absl::string_view library_name) -> std::string {
    if (!shared_library_dir) {
      return std::string(library_name);
    }
    const std::filesystem::path candidate =
        std::filesystem::path(*shared_library_dir) / std::string(library_name);
    if (std::filesystem::exists(candidate)) {
      LITERT_LOG(LITERT_INFO, "Using absolute QNN library path: %s",
                 candidate.c_str());
      return candidate.string();
    }
    LITERT_LOG(
        LITERT_WARNING,
        "Did not find %s under shared_library_dir=%s; falling back to soname "
        "lookup.",
        std::string(library_name).c_str(), shared_library_dir->c_str());
    return std::string(library_name);
  };
  ...
  LITERT_RETURN_IF_ERROR(LoadSystemLib(ResolveLibraryPath(kLibQnnSystemSo)));
  ...
  switch (options_.GetBackendType()) {
    case ::qnn::BackendType::kHtpBackend: {
      LITERT_RETURN_IF_ERROR(
          LoadLib(ResolveLibraryPath(::qnn::HtpBackend::GetLibraryName())));
      LITERT_RETURN_IF_ERROR(
    ...
    case ::qnn::BackendType::kIrBackend: {
      LITERT_RETURN_IF_ERROR(
          LoadLib(ResolveLibraryPath(::qnn::IrBackend::GetLibraryName())));
      LITERT_RETURN_IF_ERROR(
    ...
    case ::qnn::BackendType::kDspBackend: {
      LITERT_RETURN_IF_ERROR(
          LoadLib(ResolveLibraryPath(::qnn::DspBackend::GetLibraryName())));
      LITERT_RETURN_IF_ERROR(

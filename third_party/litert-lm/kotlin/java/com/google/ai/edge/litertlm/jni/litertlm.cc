...
void MarkConversationBenchmarkDelta(Conversation* conversation,
  ...
  auto benchmark_info_or = conversation->GetMutableBenchmarkInfo();
  if (!benchmark_info_or.ok() || *benchmark_info_or == nullptr) {
    return;
  }
  (*benchmark_info_or)->TimeMarkDelta(mark_name).IgnoreError();
}

OptionalArgs CreateOptionalArgs(jboolean has_pending_message,
                                jint max_output_tokens) {
  OptionalArgs optional_args;
  optional_args.has_pending_message = has_pending_message;
  if (max_output_tokens > 0) {
    optional_args.max_output_tokens = static_cast<int>(max_output_tokens);
  }
...
jobject CreateBenchmarkInfoJni(
  ...
  jmethodID benchmark_info_ctor =
      env->GetMethodID(benchmark_info_cls, "<init>",
                       "(DDIIDDLjava/lang/String;)V");
  ...
  nlohmann::ordered_json mark_durations_json = nlohmann::ordered_json::object();
  for (const auto& [mark_name, duration] : benchmark_info.GetMarkDurations()) {
    mark_durations_json[mark_name] = absl::ToDoubleMilliseconds(duration);
  }
  jstring mark_durations_json_string =
      NewStringStandardUTF(env, mark_durations_json.dump());

  jobject benchmark_info_obj = env->NewObject(
      ...
      last_decode_tokens_per_second, mark_durations_json_string);
  env->DeleteLocalRef(mark_durations_json_string);
  return benchmark_info_obj;
}
...
LITERTLM_JNIEXPORT void JNICALL JNI_METHOD(nativeSendMessageAsyncWithOptions)(
    JNIEnv* env, jclass thiz, jlong conversation_pointer,
    jstring messageJSONString, jboolean has_pending_message,
    jint max_output_tokens, jobject callback);
...
LITERTLM_JNIEXPORT void JNICALL JNI_METHOD(nativeSendMessageAsync)(
  ...
  JNI_METHOD(nativeSendMessageAsyncWithOptions)(
      env, thiz, conversation_pointer, messageJSONString,
      /*has_pending_message=*/JNI_FALSE, /*max_output_tokens=*/-1, callback);
}
...
LITERTLM_JNIEXPORT void JNICALL JNI_METHOD(nativeSendMessageAsyncWithOptions)(
  ...
  MarkConversationBenchmarkDelta(conversation, "jni_message_parse");
  ...
  auto status = conversation->SendMessageAsync(
      json_message, std::move(callback_fn),
      CreateOptionalArgs(has_pending_message, max_output_tokens));

  if (!status.ok()) {
    ThrowLiteRtLmJniException(
        env, "Failed to start nativeSendMessageAsyncWithOptions: " +
                 status.ToString());
  }
}
...
LITERTLM_JNIEXPORT jstring JNICALL JNI_METHOD(nativeSendMessage)(
  ...
  return JNI_METHOD(nativeSendMessageWithOptions)(
      env, thiz, conversation_pointer, messageJSONString,
      /*has_pending_message=*/JNI_FALSE, /*max_output_tokens=*/-1);
}
...
LITERTLM_JNIEXPORT jstring JNICALL JNI_METHOD(nativeSendMessageWithOptions)(
  ...
  auto response = conversation->SendMessage(
      json_message, CreateOptionalArgs(has_pending_message, max_output_tokens));
  if (!response.ok()) {
    ThrowLiteRtLmJniException(

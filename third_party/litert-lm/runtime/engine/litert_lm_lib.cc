...
absl::Status PrintJsonMessage(const JsonMessage& message,
  ...
  const bool event_mode = IsEventModeEnabled();
  if (message["content"].is_array()) {
    for (const auto& content : message["content"]) {
      if (content["type"] == "text") {
        ...
        if (event_mode) {
          EmitEventLine({{"type", "TOKEN"}, {"text", text}});
        } else {
          std::cout << text;
        }
      }
    }
    if (!event_mode && !streaming) {
    ...
    const std::string text = message["content"]["text"].get<std::string>();
    if (!streaming) {
      captured_output << text << std::endl << std::flush;
      if (event_mode) {
        EmitEventLine({{"type", "TOKEN"}, {"text", text}});
      } else {
      ...
      captured_output << text << std::flush;
      if (event_mode) {
        EmitEventLine({{"type", "TOKEN"}, {"text", text}});
      } else {
...
void LogBenchmarkInfo(const litert::lm::BenchmarkInfo& benchmark_info,
  ...
  if (IsEventModeEnabled()) {
    ...
    const uint64_t prefill_turns = benchmark_info.GetTotalPrefillTurns();
    const uint64_t decode_turns = benchmark_info.GetTotalDecodeTurns();
    const uint64_t max_turns =
        prefill_turns > decode_turns ? prefill_turns : decode_turns;
    for (uint64_t i = 0; i < max_turns; ++i) {
      json turn;
      if (i < prefill_turns) {
        auto prefill_turn = benchmark_info.GetPrefillTurn(static_cast<int>(i));
        if (prefill_turn.ok()) {
          turn["prefill_tokens"] = prefill_turn->num_tokens;
          turn["prefill_duration_ms"] =
              absl::ToDoubleMilliseconds(prefill_turn->duration);
          turn["prefill_tokens_per_sec"] =
              benchmark_info.GetPrefillTokensPerSec(static_cast<int>(i));
        }
      }
      if (i < decode_turns) {
        auto decode_turn = benchmark_info.GetDecodeTurn(static_cast<int>(i));
        if (decode_turn.ok()) {
          turn["decode_tokens"] = decode_turn->num_tokens;
          turn["decode_duration_ms"] =
              absl::ToDoubleMilliseconds(decode_turn->duration);
          turn["decode_tokens_per_sec"] =
              benchmark_info.GetDecodeTokensPerSec(static_cast<int>(i));
        }
      }
      turns.push_back(std::move(turn));
    }
    EmitEventLine({{"type", "BENCHMARK"},
                   {"ttft_sec", benchmark_info.GetTimeToFirstToken()},
...
void LogMemoryUsage(const LiteRtLmSettings& settings, float peak_mem_mb,
  ...
  if (IsEventModeEnabled()) {
    EmitEventLine({{"type", "MEMORY"},
                   {"peak_system_ram_mb", peak_mem_mb},
                   {"peak_private_footprint_mb", peak_private_mb}});
    return;
  }
  if (!settings.log_sink_file.has_value()) {

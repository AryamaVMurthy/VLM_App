...
std::vector<std::string> BufferNamesForDebug(
    const absl::flat_hash_map<absl::string_view, TensorBuffer>& buffers) {
...
absl::StatusOr<std::vector<absl::string_view>> ResolveSharedOutputNames(
    ...
    const absl::flat_hash_map<absl::string_view, TensorBuffer>&
        gemma_input_buffers) {
  LITERT_ASSIGN_OR_RETURN(
      auto signature_output_names,
      auxiliary_compiled_model.GetSignatureOutputNames(signature_name));

  std::vector<absl::string_view> shared_output_names;
  for (const auto& output_name : signature_output_names) {
    auto it = gemma_input_buffers.find(absl::string_view(output_name));
    if (it != gemma_input_buffers.end()) {
      shared_output_names.push_back(it->first);
    }
  }

  if (shared_output_names.empty()) {
    return absl::InvalidArgumentError(absl::StrCat(
        "No shared output names between auxiliary signature '", signature_name,
        "' and transformer inputs. Auxiliary outputs: [",
        absl::StrJoin(signature_output_names, ", "),
        "]. Transformer inputs: [",
        absl::StrJoin(BufferNamesForDebug(gemma_input_buffers), ", "), "]"));
  }

  ABSL_LOG(INFO) << "ResolveSharedOutputNames signature='" << signature_name
                 << "' auxiliary_outputs=["
                 << absl::StrJoin(signature_output_names, ", ")
                 << "] gemma_inputs=["
                 << absl::StrJoin(BufferNamesForDebug(gemma_input_buffers), ", ")
                 << "] shared_outputs=["
                 << absl::StrJoin(shared_output_names, ", ") << "]";

  return shared_output_names;
}
...
LlmLiteRtNpuCompiledModelExecutor::CreateMaskContextWithBufferSharing(
  ...
  LITERT_ASSIGN_OR_RETURN(
      ...
      ResolveSharedOutputNames(
          npu_auxiliary_context.npu_auxiliary_compiled_model,
          MaskSignatures::kPrefillMask, gemma_prefill_input_buffers));
  ...
  LITERT_ASSIGN_OR_RETURN(
      ...
      ResolveSharedOutputNames(
          npu_auxiliary_context.npu_auxiliary_compiled_model,
          MaskSignatures::kDecodeMask, gemma_decode_input_buffers));
...
LlmLiteRtNpuCompiledModelExecutor::CreateRopeContextWithBufferSharing(
  ...
  LITERT_ASSIGN_OR_RETURN(
      auto prefill_rope_output_names,
      ResolveSharedOutputNames(
          npu_auxiliary_context.npu_auxiliary_compiled_model,
          RopeSignatures::kPrefillRope, gemma_prefill_input_buffers));
  for (const auto& rope_output_name : prefill_rope_output_names) {
    LITERT_ASSIGN_OR_RETURN(
  ...
  LITERT_ASSIGN_OR_RETURN(
      auto decode_rope_output_names,
      ResolveSharedOutputNames(
          npu_auxiliary_context.npu_auxiliary_compiled_model,
          RopeSignatures::kDecodeRope, gemma_decode_input_buffers));
  for (const auto& rope_output_name : decode_rope_output_names) {
    LITERT_ASSIGN_OR_RETURN(

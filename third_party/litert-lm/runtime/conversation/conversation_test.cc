...
TEST(ConversationConfigTest, MarkerMismatchUsesPromptTemplates) {
  ASSERT_OK_AND_ASSIGN(auto model_assets,
                       ModelAssets::Create(GetTestdataPath(kTestLlmPath)));
  ASSERT_OK_AND_ASSIGN(auto engine_settings, EngineSettings::CreateDefault(
                                                 model_assets, Backend::CPU));
  ...
  auto& metadata = engine_settings.GetMutableLlmMetadata();
  metadata.mutable_prompt_templates()->mutable_user()->set_prefix(
      "<|im_start|>user\n");
  metadata.mutable_prompt_templates()->mutable_user()->set_suffix(
      "<|im_end|>\n");
  metadata.mutable_prompt_templates()->mutable_model()->set_prefix(
      "<|im_start|>assistant\n");
  metadata.mutable_prompt_templates()->mutable_model()->set_suffix(
      "<|im_end|>\n");
  metadata.mutable_prompt_templates()->mutable_system()->set_prefix(
      "<|im_start|>system\n");
  metadata.mutable_prompt_templates()->mutable_system()->set_suffix(
      "<|im_end|>\n");
  metadata.set_jinja_prompt_template(
      "{{ '<start_of_turn>user\\n' }}{{ '<end_of_turn>\\n' }}");

  ASSERT_OK_AND_ASSIGN(auto engine, EngineFactory::CreateAny(engine_settings));
  ASSERT_OK_AND_ASSIGN(auto config,
                       ConversationConfig::Builder().Build(*engine));
  EXPECT_THAT(config.GetPromptTemplate().GetTemplateSource(),
              testing::HasSubstr("<|im_start|>user"));
  EXPECT_THAT(config.GetPromptTemplate().GetTemplateSource(),
              testing::HasSubstr("<|im_start|>assistant"));
  EXPECT_THAT(config.GetPromptTemplate().GetTemplateSource(),
              testing::HasSubstr("<|im_end|>"));
  EXPECT_THAT(config.GetPromptTemplate().GetTemplateSource(),
              testing::Not(testing::HasSubstr("<start_of_turn>")));
}

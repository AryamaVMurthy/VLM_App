...
bool UsesStartOfTurnMarkers(absl::string_view text) {
  return absl::StrContains(text, "<start_of_turn>") ||
         absl::StrContains(text, "<end_of_turn>");
}

bool UsesImMarkers(absl::string_view text) {
  return absl::StrContains(text, "<|im_start|>") ||
         absl::StrContains(text, "<|im_end|>") ||
         absl::StrContains(text, "<im_start>") ||
         absl::StrContains(text, "<im_end>");
}

bool PromptTemplatesUseStartOfTurnMarkers(
    const proto::PromptTemplates& prompt_templates) {
  return UsesStartOfTurnMarkers(prompt_templates.user().prefix()) ||
         UsesStartOfTurnMarkers(prompt_templates.user().suffix()) ||
         UsesStartOfTurnMarkers(prompt_templates.model().prefix()) ||
         UsesStartOfTurnMarkers(prompt_templates.model().suffix()) ||
         UsesStartOfTurnMarkers(prompt_templates.system().prefix()) ||
         UsesStartOfTurnMarkers(prompt_templates.system().suffix());
}

bool PromptTemplatesUseImMarkers(const proto::PromptTemplates& prompt_templates) {
  return UsesImMarkers(prompt_templates.user().prefix()) ||
         UsesImMarkers(prompt_templates.user().suffix()) ||
         UsesImMarkers(prompt_templates.model().prefix()) ||
         UsesImMarkers(prompt_templates.model().suffix()) ||
         UsesImMarkers(prompt_templates.system().prefix()) ||
         UsesImMarkers(prompt_templates.system().suffix());
}

bool HasPromptTemplateMarkerFamilyMismatch(
    const proto::LlmMetadata& metadata) {
  if (!metadata.has_jinja_prompt_template() || !metadata.has_prompt_templates()) {
    return false;
  }
  const bool jinja_uses_start_of_turn =
      UsesStartOfTurnMarkers(metadata.jinja_prompt_template());
  const bool jinja_uses_im = UsesImMarkers(metadata.jinja_prompt_template());
  const bool templates_use_start_of_turn =
      PromptTemplatesUseStartOfTurnMarkers(metadata.prompt_templates());
  const bool templates_use_im =
      PromptTemplatesUseImMarkers(metadata.prompt_templates());
  return (templates_use_im && jinja_uses_start_of_turn && !jinja_uses_im) ||
         (templates_use_start_of_turn && jinja_uses_im &&
          !jinja_uses_start_of_turn);
}

absl::StatusOr<std::string> BuildPromptTemplateSourceFromMetadata(
    const proto::LlmMetadata& metadata, bool force_generic_from_prompt_templates) {
  if (!metadata.has_prompt_templates()) {
    return absl::InvalidArgumentError(
        "Prompt templates are required to build prompt template source.");
  }
  proto::LlmModelType llm_model_type = metadata.llm_model_type();
  if (force_generic_from_prompt_templates) {
...
absl::StatusOr<ConversationConfig> ConversationConfig::CreateInternal(
  ...
  if (overwrite_prompt_template.has_value()) {
    ...
    const bool has_marker_family_mismatch =
        HasPromptTemplateMarkerFamilyMismatch(*metadata);
    if (has_marker_family_mismatch) {
      ABSL_LOG(WARNING)
          << "Detected mismatch between metadata jinja markers and "
             "prompt_templates markers. Rebuilding jinja from prompt_templates "
             "using generic marker-compatible template.";
      ASSIGN_OR_RETURN(std::string jinja_source,
                       BuildPromptTemplateSourceFromMetadata(
                           *metadata, /*force_generic_from_prompt_templates=*/true));
      prompt_template = PromptTemplate(jinja_source);
    } else if (metadata->has_jinja_prompt_template()) {
      prompt_template = PromptTemplate(metadata->jinja_prompt_template());
    } else if (metadata->has_prompt_templates()) {
      ASSIGN_OR_RETURN(
          std::string jinja_source, BuildPromptTemplateSourceFromMetadata(
                                        *metadata,
                                        /*force_generic_from_prompt_templates=*/
                                        false));
      prompt_template = PromptTemplate(jinja_source);
    } else {
      return absl::InvalidArgumentError(
...
absl::Status Conversation::SendMessageAsync(
  ...
  if (auto benchmark_info_or = session_->GetMutableBenchmarkInfo();
      benchmark_info_or.ok()) {
    benchmark_info = *benchmark_info_or;
  }
  auto mark_delta = [benchmark_info](absl::string_view mark_name) {
  ...
  auto json_message = std::get<nlohmann::ordered_json>(message);
  mark_delta("conversation_render_prompt");
  ASSIGN_OR_RETURN(const std::string& single_turn_text,
                   GetSingleTurnText(message, optional_args));
  mark_delta("conversation_render_prompt");
  {
  ...
  mark_delta("conversation_to_input_data");
  ASSIGN_OR_RETURN(
      const auto session_inputs,
      model_data_processor_->ToInputDataVector(
          single_turn_text, nlohmann::ordered_json::array({json_message}),
  ...
  ASSIGN_OR_RETURN(
      ...
      CreateDecodeConfig(std::move(optional_args.decoding_constraint),
                         optional_args.max_output_tokens));
  if (is_appending_message_) {
    mark_delta("conversation_prefill_schedule");
    ASSIGN_OR_RETURN(
    ...
    mark_delta("conversation_prefill_schedule");
    mark_delta("conversation_prefill_wait");
    ASSIGN_OR_RETURN(
        ...
        session_->RunPrefillAsync(
            session_inputs, [this, callback = internal_callback, decode_config,
              ...
              if (!status.ok()) {
                ...
                if (benchmark_info != nullptr) {
                  benchmark_info
                      ->TimeMarkDelta("conversation_prefill_wait")
                      .IgnoreError();
                  benchmark_info
                      ->TimeMarkDelta("conversation_decode_to_first_chunk")
                      .IgnoreError();
                }
                auto is_first_decode_callback =
                    std::make_shared<bool>(true);
                auto decode_task_controller = session_->RunDecodeAsync(
                    [callback, benchmark_info,
                     is_first_decode_callback](
                        absl::StatusOr<Responses> responses) {
                      if (benchmark_info != nullptr &&
                          *is_first_decode_callback) {
                        benchmark_info
                            ->TimeMarkDelta(
                                "conversation_decode_to_first_chunk")
                            .IgnoreError();
                        *is_first_decode_callback = false;
                      }
                      (*callback)(responses);
                    },
                    decode_config);
